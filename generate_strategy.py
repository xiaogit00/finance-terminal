...
def main():
    # Define paths
    source_folder = "utils/strategy_generation_template"
    models_folder = "strategies"
    ...
    models_dir = BASE_CODE_DIR / "strategies"
    ...
    strategy_name = get_valid_input(
        "Strategy Name: ",
        pattern=r'^[a-zA-Z0-9_\-\s]+$',
        example="SMALongShortCrossover (in camelCase)"
    )
    
    # Model name (alphanumeric, spaces, hyphens, underscores)
    ticker_name = get_valid_input(
        "Ticker Name: ",
        pattern=r'^[a-zA-Z0-9_\-\s]+$',
        example="AAPL (or a group of tickers like MAANG)"
    )
    ...
    dataset_name_clean = strategy_name.replace(' ', '_')
    model_name_clean = ticker_name.replace(' ', '_')
